(*
 *
 * AoF - Testbench for the Solution of Day 11
 * Created:     2026-01-03
 * Modified:    2026-01-03
 * Author:      Kagan Dikmen
 *
...
let%expect_test "day11_test" =
  let cycles_per_bit = 4 in

  let stream = In_channel.read_all "toy_input.txt"
    |> String.to_list
  ...
  let part_1, part_2, is_done, debug_output = Day11.create_day11_logic
    ~clock
    ~clear
    ~cycles_per_bit
    uart_rx_value
